...
public class MultiLoaderPluginProcessService extends BasePluginProcessService {
    ...
    synchronized void loadRuntimeForPlugin(String pluginKey, String uuid) throws FailedException {
        ...
        if (isRuntimeLoaded(pluginKey)) {
            throw new FailedException(FailedException.ERROR_CODE_RELOAD_RUNTIME_EXCEPTION, "重复调用loadRuntime," + logIdentity);
        }
        try {
            ...
            try {
                ...
                throw new FailedException(FailedException.ERROR_CODE_UUID_MANAGER_DEAD_EXCEPTION, e.getMessage());
            } catch (NotFoundException e) {
                Log.i("PluginProcessService", "uuidManager.getRuntime new NotFoundException");
                throw new FailedException(FailedException.ERROR_CODE_FILE_NOT_FOUND_EXCEPTION, "pluginKey=" + pluginKey + ", uuid=" + uuid + "的Runtime没有找到。cause:" + e.getMessage());
            }
    ...
    synchronized void loadPluginLoaderForPlugin(String pluginKey, String uuid) throws FailedException {
        ...
        if (mPluginLoaderMap.get(pluginKey) != null) {
            throw new FailedException(FailedException.ERROR_CODE_RELOAD_LOADER_EXCEPTION, "重复调用loadPluginLoader");
        }
        try {
            InstalledApk installedApk;
            try {
                ...
                throw new FailedException(FailedException.ERROR_CODE_FILE_NOT_FOUND_EXCEPTION, logIdentity + "的PluginLoader没有找到。cause:" + e.getMessage());
            }
            File file = new File(installedApk.apkFilePath);
            if (!file.exists()) {
                throw new FailedException(FailedException.ERROR_CODE_FILE_NOT_FOUND_EXCEPTION, file.getAbsolutePath() + "文件不存在");
            }
            ...
            throw new FailedException(FailedException.ERROR_CODE_RUNTIME_EXCEPTION, "加载动态实现失败 cause：" + msg);
        }
    }
    ...
    private UuidManager checkUuidManagerNotNull(String pluginKey) throws FailedException {
        ...
        if (uuidManager == null) {
            throw new FailedException(FailedException.ERROR_CODE_UUID_MANAGER_NULL_EXCEPTION, "mUuidManager == null");
        }
        return uuidManager;
    }
    ...
    private void addUuidForPlugin(String pluginKey, String uuid) throws FailedException {
        ...
        if (preUuid != null && !TextUtils.equals(uuid, preUuid)) {
            throw new FailedException(FailedException.ERROR_CODE_RESET_UUID_EXCEPTION, "Plugin=" + pluginKey + "已设置过uuid==" + preUuid + ", 试图设置uuid==" + uuid);
